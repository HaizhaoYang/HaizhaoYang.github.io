...
function dbchdav2(varargin...;pids, verb=false)
    ...
    global elapsedTime = Dict("total"=>0.0, "dCheb_filter"=>0.0, "dCheb_filter_n"=>0.0, "dCheb_filter_scal"=>0.0,
                ...
                "dDGKS_@distributed_computingnorm"=>0.0, "dDGKS_@distributed_computingnorm_n"=>0.0, "construction"=>0.0
    )
    ...
    elapsedTime["total"] = @elapsed begin
        ...
        elapsedTime["construction"] = @elapsed begin
            DA_operator = varargin[1]
            dim = size(DA_operator,1)
            @eval @everywhere edim = $dim
            wkers = DA_operator.pids
            nwkers = length(wkers)
            @mpi_do pids EA_operator = Elemental.DistSparseMatrix(Float64, edim, edim)
            @sync for j in 1:nwkers
                @spawnat wkers[j] begin
                    row, col, val = findnz(localpart(DA_operator))
                    for jj in 1:length(row)
                        rw = row[jj]
                        cl = col[jj]
                        vl = val[jj]
                        Elemental.queueLocalUpdate(EA_operator, rw, cl, vl)
                    end
                end
            end
            @mpi_do pids Elemental.processQueues(EA_operator)
        end 
        ...
        if  haskey(opts, "v0")
            ...
            # @mpi_do pids eV0 = Elemental.DistMultiVec(Float64)
            # @mpi_do pids Elemental.gaussian!(eV0, edim, eblk)
            sizev0 = blk
        end
        end
        ...
        elapsedTime["dzeros"] += @elapsed begin
            ...
            # @mpi_do pids V = Elemental.DistMultiVec(Float64)
            # @eval @mpi_do pids Elemental.zeros!(V, edim, $vomax)
            # @mpi_do pids W = Elemental.DistMultiVec(Float64)
            # @eval @mpi_do pids Elemental.zeros!(W, edim, trunc(Int64, $vimax))
        end
        ...
        elapsedTime["main_loop"] = @elapsed begin
            while  iter_total <= itmax 
                ...
                elseif filter == 2
                    ...
                    @eval @everywhere e = ($upb - $low_nwb)/2.0
                    @eval @everywhere center = ($upb+$low_nwb)/2.0
                    @eval @everywhere sigma = e/($lowb - center)
                    @eval @everywhere tau = 2.0/sigma
